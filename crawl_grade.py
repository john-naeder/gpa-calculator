...
def generate_sql_script(grades, output_file):
    """Generate SQL script to insert or update grades in the database."""
    with open(output_file, 'w', encoding='utf-8') as file:
        file.write("-- Auto-generated SQL script for grade insertion/update\n")
        file.write("-- Generated by crawl_grade.py\n")
        file.write("-- Simplified version without existence checks\n\n")
        
        file.write("START TRANSACTION;\n\n")
        
        file.write("-- Temporarily disable foreign key checks\n")
...
def main():
    parser = argparse.ArgumentParser(description='Crawl grades from HTML file and generate SQL script.')
    ...
    parser.add_argument('-s', '--sql-style', choices=['simple', 'duplicate-key'], default='simple',
                      help='SQL generation style: simple inserts or ON DUPLICATE KEY UPDATE (default: simple)')
    ...
    if grades:
        ...
        if args.sql_style == 'duplicate-key':
            try:
                from crawl_grade_alt import generate_sql_script as generate_duplicate_key_sql
                generate_duplicate_key_sql(grades, args.output)
            except ImportError:
                print("Warning: Could not import ON DUPLICATE KEY function. Falling back to simple INSERT.")
